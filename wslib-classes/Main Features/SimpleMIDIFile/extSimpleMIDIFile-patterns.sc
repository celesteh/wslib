...
//// the following methods were kindly added by Jascha Narveson
	...
	generatePatternSeqs { |useTempo = true| // amp: amp when velo == 127
		var thisFile;
		//inst = ( inst ? 'default' ).asCollection;
		...
		^this.tracks.collect({ |tr|
			var sustainEvents, deltaTimes, notes, durs, arr, chord, polyphonic;
			sustainEvents = thisFile.noteSustainEvents( nil, tr );
			( sustainEvents.size > 0 ).if(
				{
					sustainEvents = sustainEvents.flop;
					( useTempo ).if( {
						deltaTimes = sustainEvents[1].differentiate;
					...
					notes = [\rest] ++ sustainEvents[4];
					durs =  deltaTimes ++ [0];

					arr = notes.collect({|note, index| [note, durs[index]] });
					polyphonic = [];
					chord = [];

					arr.do({|event|

						(event.last == 0).if ({ // does the note have no duration?
							(event.first != \rest).if({ // don't bother with rests
								chord = chord.add(event.first); // save it for the next event with a duration
							});
							} , { // we've got a duration
								(chord.size > 0).if({ // are there notes for the chord?
									event[0] = chord ++ event.first; // make an array of notes
								});
								chord = [];
								polyphonic = polyphonic.add(event);
						})
					});

					polyphonic; // return the array
...
Returns an array of [note, dur] sequence info for each track, with proper accounting of rests. �
...
Patterns wishing to involve several tracks should be sure to specify \track and \channel info in the pattern itself. �

Currently, only pitch and rhythm and dynamic info is rendered.� Pitch may be specified in any of the usual pattern ways, as \freq or \midinote or \degree.� The \amp key is used to generate velocity information, with the 0-1 range of amp mapped to the normal midi 0-127.

Example:

~high = Pbind(
	\track, 1, \channel, 0, \degree, Pwhite(-8, 8), \octave, 6, \scale, #[0,2,3,5,7,8,10],�
